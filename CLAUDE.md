...
Gioco browser di gestione carriera tennistica ATP. Nessun server, nessun build
step: HTML + React 18 (CDN) + Babel standalone. Salvataggio in localStorage
(chiave `atp_career_v3`). Deve funzionare aprendo `index.html` con doppio click
(`file://`) e su Safari iOS.
...
- **Formato salvataggio**: non cambiare la chiave `atp_career_v3` né rimuovere
  campi dallo state senza migrazione (vedi `migrateWorldPool`).
...
```bash
node --check js/game_data.js && node --check js/engine.js && node --check js/npc_system.js
