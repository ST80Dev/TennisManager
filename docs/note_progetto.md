...
```
  ...
  ATP500:      rounds=6, draw=64,  180.000,   500pts, minRank 150, rankFloor 30
  ATP250:      rounds=6, draw=64,  120.000,   250pts, minRank 315, rankFloor 30,  rankCeiling 60
  ChallengerA: rounds=5, draw=32,   35.000,    90pts, minRank 370, rankFloor 100, rankCeiling 150
  ChallengerB: rounds=5, draw=32,   15.000,    44pts, minRank 450, rankFloor 250, rankCeiling 250
  ITF:         rounds=4, draw=16,    8.000,     6pts, minRank 500, rankFloor 350, rankCeiling 370
  ATPFinals:   rounds=6, draw=8,  500.000,   1500pts, minRank 8,   rankFloor 1
...
```
  ...
  ATP 250:      rank 60–315   ← ceiling abbassato da 100 a 60
  ATP 500:      rank 1–150
...
```
  ...
  LODGE_BASE:   ITF:50 · ChalB:75 · ChalA:100 · ATP250:400 · ATP500:1000
                ATPFinals:1000 · M1000:2500 · Slam:5000
  LODGE_COLLAB: ITF:10 · ChalB:20 · ChalA:40 · ATP250:70 · ATP500:150
                M1000:350 · Slam:700
...
```
  ...
  dimFactor = max(0.15, ((35-stat)/35)^0.6)
...
```
  ...
  TALENTO (nuovo, sostituisce initRank/3 e initRank*8):
    7 tiers, ciascuno con ceilRank/floorRank ASSOLUTI:
      Legend   ceil 1   floor 18
      Elite    ceil 3   floor 35
      Great    ceil 8   floor 65
      VeryGood ceil 15  floor 130
      Good     ceil 35  floor 220
      Solid    ceil 85  floor 360
      Journeyman ceil 180 floor 500
    Tier originali (ATP400) → dedotto da initRank.
    Tier newgen (replacement) → distribuzione random pesata:
      3% Legend · 6% Elite · 11% Great · 15% VeryGood · 20% Good · 25% Solid · 20% Journeyman
    Hidden talent: 10% degli NPC rank 41-200 ricevono upgrade di un tier
      → candidati breakout (rank 80 Good → VeryGood → ceil 15).
    API: getNPCCareerBounds(npcId, initRank) → {tier, ceilPts, floorPts}

  CAREER PHASE (nuovo, pluriennale):
    steady (default) · breakthrough (+drift ×1.6, bias +0.65) ·
    slump (+drift ×1.5, bias -0.65)
    Transizioni a isNewYear:
      steady → breakthrough: 9% (<26) · 4% (26-29) · 1% (30+)
      steady → slump:        3% (<28) · 6% (28-31) · 9% (32+)
      breakthrough → steady: 28%/anno
      slump → steady:        22%/anno
    Stato: npcCareerPhase[id] (persistito in save come gs.npcState.careerPhase)

  RETIREMENT (esteso):
    Base distribution (getRetireAge):
      30% 32-34 · 55% 35-37 · 12% 38-40 · 3% 41-43
    Early retirement stocastica (shouldEarlyRetire):
      age≥25: +0.4%  · age≥30: +0.6%  · age≥33: +1.0% (infortunio)
      age≥27 + 3+ anni fuori top150: +12% (burnout)
      age≥29 + 2+ anni fuori top250: +18% (decisione)
      age≥32 + rank>200: +10%
      age≥35 + rank>100: +15%
    Contatori: npcYearsOutside150[id], npcYearsOutside250[id]
    Tutti persistiti in gs.npcState.

  NEWGEN (replacement) — rank di partenza graduato dal tier:
    Legend 180-250 · Elite 240-330 · Great 280-370 · VeryGood 320-390
    Good 350-400 · Solid/Journeyman 370-400
    Età: Legend/Elite 16-19 · altri 17-19

  GAIN CAP (più aggressivo per i giovani):
    rank≤10: 500 · ≤30: 200 · ≤80: 95 · ≤150: 42 · ≤250: 22 · altri: 14
    Bonus giovani: <21 ×3.0 · <24 ×2.0 · <27 ×1.3 (nuovo)

  MR: rank≤20: 0.050 · rank≤100: 0.035 · rank≤300: 0.045 · altri: 0.060
...
```
  ...
  reale di simWeek (vedi VALIDAZIONE sotto).

  BUG RISOLTI (68-72, vedi sezione BUG):
    68. driftSeed annuale costante · 69. rumore settimanale morto (sv(NaN))
    70. newgen di talento declassati Journeyman · 71. ageBias MR inverso
    72. leak npcForm/npcPersonalLevel dei ritirati

  NUOVE MECCANICHE (npc_system.js):
    youngGrowthRate(age): frazione ANNUA del gap verso il ceiling REALIZZATO
      recuperata dai giovani (motore del ricambio generazionale):
      <18: 0.10 · <19: 0.20 · <21: 0.30 (picco) · <23: 0.20 · <25: 0.12 · <27: 0.06
      Applicata in simWeek come rate/51 a settimana.
    talentRealization(id): r ∈ [0.45, 1.0] personale — la crescita punta a
      floor + r×(ceil−floor), NON al ceiling pieno. In fase "breakthrough"
      il target sale al ceiling pieno → le career phase decidono chi sfonda.
    getEffectiveFloorPts(floor, age): il floor di talento sfuma ×0.60/anno
      oltre i 30 anni → i veterani possono davvero uscire dal top 20/100
      (prima un Legend restava top-18 a vita fino al ritiro).
    drift_mod di getAgeParams ORA USATO (prima calcolato e ignorato):
      moltiplica il personalLevel ogni settimana → declino progressivo
      30+ (fino a -14/-34% l'anno a 35+), personalizzato dal char.
    win_mod di getAgeParams ORA USATO in simNPCBracket/atpFinalsSimMatch:
      giovani sovraperformano il rank nei tornei, veterani sottoperformano.
    getTalent(id, initRank, isNewgen, initAge): upgrade giovinezza per
      ORIGINALI giovani a rank modesto (initRank>20): initAge≤21 → 45% +1
      tier (15% +2) · 22-24 → 25% +1. initAge stampato in buildWorld e
      generateReplacementNPC (old save senza initAge → nessun upgrade, ok).
    tierFromRandom RICALIBRATA: 1% Legend · 3% Elite · 7% Great · 14% VG ·
      22% Good · 28% Solid · 25% Journeyman (era 3/6/11/15/20/25/20: con
      ~38 ritiri/anno produceva 1 Legend e 2 Elite OGNI anno → vetta
      affollata e curva punti compressa).
    getDecayRate RIMOSSA (codice morto, superseded dalla MR di Phase 1).

  PARAMETRI RITOCCATI (simWeek Phase 1):
    driftPct annuale per archetipo: 0.10/0.13/0.15/0.19/0.25 (era 0.08-0.20)
    biasedDrift = clamp(raw×0.7 + signBias×0.5) (era raw×0.5 — ampiezza piena)
    rumore settimanale: ±0.6% (≤30) · ±1.0% (≤100) · ±1.4% (era 0.3/0.6/1.0)
    Phase 1b NUOVA: rinormalizzazione settimanale dei personalLevel a
      NPC_FIXED_POOL — la crescita giovani immette più livello del declino
      veterani; senza rescale la curva punti si scolla da atpPtsForRank
      (ordinale invariato, scala punti preservata per sponsor/UI).
    MODALITÀ LEGGENDA: Phase 1 evolutiva GATED (niente drift annuale, niente
      growth/decline settimanale, floor pieno) → livelli cristallizzati.
      Verificato: top10 identica dopo 3 anni, età ferme.

  VALIDAZIONE (harness 10 anni × 3 repliche, target = ATP reale):
    top10 retention: y1 0.90 · y3 0.80 · y5 0.40 · y10 0.00
    top100 retention: y1 0.79 · y3 0.50 · y5 0.27 · y10 0.04
    n.1 distinti (settimanali) in 10 anni: ~5 · top10 distinti: ~36
    breakout giovani (u22 rank 30-300, +60 pos in 3 anni): ~30%
    declino 30+ top100 (−30 pos o ritiro in 3 anni): ~100%
    miglior newgen dopo 10 anni: #1 · newgen in top50: ~48/50
    ritiri: ~38/anno
...
```
  ...
  Causa: player.rank calcolato come "NPC con più pts + 1", senza riassegnare
         il rank degli NPC sottostanti.
  Fix:  shiftNpcRanksForPlayer(world, playerRank, playerPts) chiamato dopo
        ogni simWeek, dopo migrateWorldPool, dopo "Rigenera ATP Rankings".
        Risultato: world ha NPC in ranks {1..501}\{playerRank}, giocatore
        occupa playerRank. Zero collisioni.
...
```
  ...
  "legend"   — NPC cristallizzati: età mai incrementata, nessun decay fisico,
               nessun ritiro. Alcaraz/Sinner/Djokovic rimangono ai livelli
               attuali per tutta la carriera. Il GIOCATORE invecchia normalmente.
  Selezionabile nella schermata di creazione personaggio (step stat).
  Salvato nel save come gs.gameMode. In GameScreen: const legendMode = ...
...
```
  ...
  Fix: gate probabilistico PRIMA del weighted sample:
    rank≤200: 3.5% per settimana → ~1-2 eventi/stagione
    rank 200-350 + awayFromHome: 1.5% per settimana
...
```
  DrawScreen: sorteggio animato stile B (seeded prima, poi non-seeded).
    ...
    Per draw ≥64: mostra sezione da 32 slot intorno al giocatore.
    Prop playerNat passata per mostrare bandiera del giocatore.
    Container: height:100vh + overflow:hidden per footer fisso.
  BracketUpdateScreen: tabellone aggiornato post-match (obbligatorio,
    con pulsante Salta). Mostra risultati round + prossimo avversario
    o celebrazione fine torneo. Sostituisce BracketTransition inline.
...
```
  ...
  Tre stati dinamici per riga:
    ...
    ~ giallo = entro 15 rank (quasi raggiungibile o a rischio perdita)
...
- Stats e Forza per superficie: in schermata Profilo (non Calendario)
- Storico partite: raggruppato per torneo, card collassabili, cap 30 tornei
- Pool nomi: 20 per nazione per ridurre collisioni fascia Challenger/ITF
- Entry fee: graduale dal basso (30/80/300/1000/2700/6000/10000)
- Gain allenamento +14-17%: top 10 raggiungibile a 25-26 anni
- TrainBonus coach: base 1.61 / esperto 1.90 / elite 2.24
...
- Fix log_flight frequenza: gate 3.5%/1.5% prima del weighted sample
- Volo privato: pulsante nel banner blockZoneChange, costo per rank
- Tabella soglie tornei: pannello collassabile nel calendario con 3 stati
- Modalità Leggenda: NPC età cristallizzata, selezionabile alla creazione
- Anno reale 2025+(year-1): visualizzato in tutta la UI al posto di "Anno X"
- Import salvataggio alla schermata iniziale (con e senza save esistente)
