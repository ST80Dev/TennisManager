...
finanze. L'obiettivo e' costruire una carriera sostenibile e arrivare ai
vertici del tennis mondiale.

Scegli quando iniziare: a 15 anni parti da zero (fuori classifica) con una
...
```
  ⏳ Classica — Gli NPC invecchiano stagione dopo stagione e si ritirano
    ...
    I giovani talenti emergono e scalano la classifica (i predestinati
    esplodono tra i 18 e i 21 anni), i veterani calano progressivamente
    dopo i 30 fino al ritiro. La modalita' piu' realistica.

  ⭐ Leggenda — Gli NPC rimangono per sempre ai livelli attuali.
    Eta' cristallizzata: nessun decay fisico, nessun ritiro.
...
```
  ...
  Esempi:
    4 allenamenti + 1 riposo  ->  fatica contenuta (ottimale)
    3 allenamenti + 2 riposi  ->  fatica quasi azzerata (ideale pre-Slam)
...
```
  ...
  Per rank ≤200: e' disponibile l'opzione "Volo privato" a pagamento
...
```
  ...
  Entry fee per categoria:
    Challenger B 80 · Challenger A 300 · ATP250 1.000
    ATP500 2.700 · M1000 6.000 · Slam 10.000

  Alloggio (giocatore + per collaboratore):
...
```
  ...
  Scala rank-based:
    Rank > 350:    55/sett  — semi-professionista
    Rank 301-350:  75/sett  — emerging player
    Rank 251-300:  95/sett  — challenger stabile
    Rank 201-250: 120/sett  — circuito challenger
    Rank 151-200: 155/sett  — circuito challenger avanzato
...
```
  ...
  Le offerte sono DINAMICHE: ogni volta che apri la schermata sponsor,
  importi, scadenze e obiettivi vengono ricalcolati in base ai tuoi PUNTI ATP
  e alla settimana corrente. Gli obiettivi sono espressi in punti ATP, non rank.

  IL CAUTO — finestra lunga (+14 sett), obiettivo punti +8% con bonus fisso
    Anticipo moderato · Restituzione 65% se fallisci · Bonus 10% se riesci
    Ideale per chi parte da 0 pts: target di pochi punti, raggiungibile al primo anno.

  L'AMBIZIOSO — finestra stretta (+7 sett), obiettivo punti +25% con bonus fisso
    Anticipo alto · Restituzione 105% se fallisci (perdi di piu' dell'anticipo)
    Da non sottovalutare: se non sei sicuro di crescere rapidamente, e' rischioso.

  IL GRADUALE — finestra lunga (+14 sett), obiettivo punti +15% con bonus fisso
    Checkpoint a meta' (+7 sett): target +5%, restituzione parziale 15% se mancato
    Restituzione 95% se fallisci il finale · Bonus 20% se riesci
    La scelta piu' equilibrata per i primi anni.
...
```
  ...
  Challenger A  — rank #150-370 · 35.000 prize · 90pts titolo
                  Primo circuito vero: punti ATP significativi da SF in su.

  ZONA DI SCELTA rank #150-370: entrambi i Challenger accessibili.

  ATP 250       — rank #60-315 · 120.000 prize · 250pts
                  Accessibile anche a rank 61-100 (non solo fino a 100).
  ATP 500       — rank <= 150 · 180.000 prize · 500pts
  Masters 1000  — rank <= 100 · 300.000 prize · 1000pts
  Grand Slam    — rank <= 104 · 700.000 prize · 2000pts

  SOGLIE DI CARRIERA SIGNIFICATIVE:
    Fuori cls (0-4 pts):   solo ITF accessibili
    Rank ~480 (12 pts):    accesso regolare Challenger B
    Rank ~350 (81 pts):    accesso Challenger A aperto
    Rank 315:              accesso ATP 250 — primo torneo del circuito principale
    Rank 150:              accesso ATP 500 — prize money quasi triplo
    Rank 100:              accesso Masters 1000 — elite del circuito
    Rank 60:               ceiling ATP 250 — puoi ancora giocare ATP 250
    Rank  50:              sponsor premium disponibili
    Rank  20:              seed fisso negli Slam, Top 20 mondiale

  TABELLA SOGLIE NEL CALENDARIO:
    Il calendario mostra un pannello collassabile "🏆 Soglie accesso tornei"
    con tutte le fasce dinamiche. I tier sono colorati in base al tuo rank:
...
di qualche decimo di stat in piu'.
...
- Schermata iniziale: tocca "Carica Salvataggio" per aprire la lista dei
  5 slot, con nome del salvataggio, giocatore, età, posizione in classifica,
  anno/settimana e data. Tocca uno slot per riprendere la partita,
  il cestino per eliminarlo.
...
```
  ...
  Profilo → "Importa file di backup" (sovrascrive lo slot attivo)
```
